...
int main()
{
	...
	int computerNum,            // The computer's randomly generated number
		guess, numGuesses = 0;                  // The user's current guess
	...
	while (guess != computerNum)
	{
		if (guess > computerNum)
		{
			...
			numGuesses++;                  //numGuesses adds the number of guesses the user took. It adds 1 each time the user goes through the while loop.
		}
	...
	cout << "\nCongratulations. You got it. My number was " << computerNum << " it took you " << numGuesses << " guess(es).\n";
	system("pause");
	return 0;
}